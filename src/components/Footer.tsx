    ...
    <footer className="bg-[#0A0A0A]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            <a href="#" className="flex items-center gap-1 select-none mb-3">
              <span className="text-gradient-green font-bold text-xl">★</span>
          ...
          <nav className="flex flex-wrap gap-x-6 gap-y-3">
            {navLinks.map((link) => (
              <button
                ...
                className="text-sm text-[#A0A0A0] hover:text-white transition-colors bg-transparent border-none cursor-pointer p-0"
              >
                {link.label}
              </button>
            ))}
